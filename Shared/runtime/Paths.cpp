...
namespace Shared::RuntimePaths {
    namespace {
        ...
        void AddUniquePath(
            std::vector<std::filesystem::path> &out, const std::filesystem::path &candidate
        ) {
        ...
        void AddPathAndParents(
            std::vector<std::filesystem::path> &out, const std::filesystem::path &start
        ) {
            if (start.empty()) {
                return;
            }
    ...
    void Initialize(const std::filesystem::path &executablePath) {
        ...
        nextState.modelsBase =
            NormalizePath(FindModelsBase(nextState.roots, nextState.voxelOpsBase));
        nextState.sharedBase =
            NormalizePath(FindSharedBase(nextState.roots, nextState.voxelOpsBase));
