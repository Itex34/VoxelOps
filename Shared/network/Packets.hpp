...
struct GrappleRequest {
    uint32_t clientGrappleId = 0;
    uint32_t clientTick = 0;
    float posX = 0.f, posY = 0.f, posZ = 0.f;
    float dirX = 0.f, dirY = 0.f, dirZ = 0.f;
    uint32_t seed = 0;
    std::vector<uint8_t> serialize() const;
    static std::optional<GrappleRequest> deserialize(const std::vector<uint8_t> &buf);
};
...
struct GrappleResult {
    ...
    uint8_t didHit = 0;
    int32_t hitEntityId = -1;
    float hitX = 0.f, hitY = 0.f, hitZ = 0.f; // world coordinates of grappled block
    uint8_t faceNormal = 0; // a block has 6 faces, so this is a value from 0 to 5 representing
                            // which face was hit, and 255 if no hit

    uint32_t serverSeed = 0;
    std::vector<uint8_t> serialize() const;
    static std::optional<GrappleResult> deserialize(const std::vector<uint8_t> &buf);
};
