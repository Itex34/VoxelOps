...
std::vector<uint8_t> ConnectRequest::serialize() const {
    const std::string identityTrimmed =
        identity.substr(0, std::min(identity.size(), kMaxConnectIdentityChars));
    const std::string nameTrimmed =
        requestedUsername.substr(0, std::min(requestedUsername.size(), kMaxConnectUsernameChars));
...
std::optional<ConnectRequest> ConnectRequest::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::ConnectRequest))
        return std::nullopt;

    ConnectRequest req;
    if (!read_u16(buf, off, req.protocolVersion))
        return std::nullopt;
    if (!read_u8(buf, off, identityLen))
        return std::nullopt;
    if (!read_u8(buf, off, nameLen))
        return std::nullopt;
    if (identityLen > kMaxConnectIdentityChars || nameLen > kMaxConnectUsernameChars)
        return std::nullopt;
    if (off + identityLen + nameLen > buf.size())
        return std::nullopt;
...
std::vector<uint8_t> ConnectResponse::serialize() const {
    const std::string assignedTrimmed =
        assignedUsername.substr(0, std::min(assignedUsername.size(), kMaxConnectUsernameChars));
    const std::string messageTrimmed =
        message.substr(0, std::min(message.size(), kMaxConnectMessageChars));
...
std::optional<ConnectResponse> ConnectResponse::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::ConnectResponse))
        return std::nullopt;

    ConnectResponse resp;
    if (!read_u8(buf, off, ok))
        return std::nullopt;
    if (!read_u8(buf, off, reason))
        return std::nullopt;
    if (!read_u16(buf, off, resp.serverProtocolVersion))
        return std::nullopt;
    if (!read_u8(buf, off, assignedLen))
        return std::nullopt;
    if (!read_u8(buf, off, msgLen))
        return std::nullopt;
    if (assignedLen > kMaxConnectUsernameChars || msgLen > kMaxConnectMessageChars)
        return std::nullopt;
    if (off + assignedLen + msgLen > buf.size())
        return std::nullopt;
...
std::optional<PlayerInput> PlayerInput::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::PlayerInput))
        return std::nullopt;

    PlayerInput p;
    if (!read_u32(buf, off, p.inputTick))
        return std::nullopt;
    if (!read_u8(buf, off, p.inputFlags))
        return std::nullopt;
    if (!read_u8(buf, off, p.flyMode))
        return std::nullopt;
    if (!read_u16(buf, off, p.weaponId))
        return std::nullopt;
    if (!read_f32(buf, off, p.yaw))
        return std::nullopt;
    if (!read_f32(buf, off, p.pitch))
        return std::nullopt;
    if (!read_f32(buf, off, p.moveX))
        return std::nullopt;
    if (!read_f32(buf, off, p.moveZ))
        return std::nullopt;
    if (!std::isfinite(p.yaw) || !std::isfinite(p.pitch) || !std::isfinite(p.moveX) ||
        !std::isfinite(p.moveZ)) {
        return std::nullopt;
    }
    return p;
}
...
std::optional<ShootRequest> ShootRequest::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::ShootRequest))
        return std::nullopt;

    ShootRequest r;
    if (!read_u32(buf, off, r.clientShotId))
        return std::nullopt;
    if (!read_u32(buf, off, r.clientTick))
        return std::nullopt;
    if (!read_u16(buf, off, r.weaponId))
        return std::nullopt;
    ...
    if (!read_u32(buf, off, r.seed))
        return std::nullopt;
    if (!read_u8(buf, off, r.inputFlags))
        return std::nullopt;
...
std::vector<uint8_t> ShootResult::serialize() const {
    ...
    write_f32(out, hitY);
    write_f32(out, hitZ);
    write_f32(out, normalX);
    write_f32(out, normalY);
    write_f32(out, normalZ);
...
std::optional<ShootResult> ShootResult::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::ShootResult))
        return std::nullopt;
    ...
    if (!read_f32(buf, off, r.hitZ))
        return std::nullopt;
    if (!read_f32(buf, off, r.normalX))
        return std::nullopt;
    if (!read_f32(buf, off, r.normalY))
        return std::nullopt;
    if (!read_f32(buf, off, r.normalZ))
        return std::nullopt;
    if (!read_f32(buf, off, r.damageApplied))
        return std::nullopt;
    if (!read_u16(buf, off, r.newAmmoCount))
        return std::nullopt;
...
std::optional<PlayerSnapshotFrame>
PlayerSnapshotFrame::deserialize(const std::vector<uint8_t> &buf) {
    size_t off = 0;
    uint8_t type = 0;
    uint32_t count = 0;
    constexpr size_t kEntrySize = 8 + (8 * 4) + 3 + 2 + 4 + 1 + 4 + 1 + 4 + 4;
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::PlayerSnapshot))
        return std::nullopt;

    PlayerSnapshotFrame frame;
    if (!read_u32(buf, off, frame.serverTick))
        return std::nullopt;
    if (!read_u64(buf, off, frame.selfPlayerId))
        return std::nullopt;
    if (!read_u32(buf, off, frame.lastProcessedInputTick))
        return std::nullopt;
    if (!read_u32(buf, off, count))
        return std::nullopt;
    if (count > ((buf.size() - off) / kEntrySize))
        return std::nullopt;
    ...
    for (uint32_t i = 0; i < count; ++i) {
        PlayerSnapshot p{};
        if (!read_u64(buf, off, p.id))
            return std::nullopt;
        if (!read_f32(buf, off, p.px))
            return std::nullopt;
        if (!read_f32(buf, off, p.py))
            return std::nullopt;
        if (!read_f32(buf, off, p.pz))
            return std::nullopt;
        if (!read_f32(buf, off, p.vx))
            return std::nullopt;
        if (!read_f32(buf, off, p.vy))
            return std::nullopt;
        if (!read_f32(buf, off, p.vz))
            return std::nullopt;
        if (!read_f32(buf, off, p.yaw))
            return std::nullopt;
        if (!read_f32(buf, off, p.pitch))
            return std::nullopt;
        if (!read_u8(buf, off, p.onGround))
            return std::nullopt;
        if (!read_u8(buf, off, p.flyMode))
            return std::nullopt;
        if (!read_u8(buf, off, p.allowFlyMode))
            return std::nullopt;
        if (!read_u16(buf, off, p.weaponId))
            return std::nullopt;
        if (!read_f32(buf, off, p.health))
            return std::nullopt;
        if (!read_u8(buf, off, p.isAlive))
            return std::nullopt;
        if (!read_f32(buf, off, p.respawnSeconds))
            return std::nullopt;
        if (!read_u8(buf, off, p.jumpPressedLastTick))
            return std::nullopt;
...
std::optional<ChunkRequest> ChunkRequest::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::ChunkRequest))
        return std::nullopt;

    ChunkRequest req;
    if (!read_i32(buf, off, req.chunkX))
        return std::nullopt;
    if (!read_i32(buf, off, req.chunkY))
        return std::nullopt;
    if (!read_i32(buf, off, req.chunkZ))
        return std::nullopt;
    if (!read_u16(buf, off, req.viewDistance))
        return std::nullopt;
    return req;
}
...
std::optional<ChunkData> ChunkData::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::ChunkData))
        return std::nullopt;

    ChunkData d;
    if (!read_i32(buf, off, d.chunkX))
        return std::nullopt;
    if (!read_i32(buf, off, d.chunkY))
        return std::nullopt;
    if (!read_i32(buf, off, d.chunkZ))
        return std::nullopt;
    if (!read_u64(buf, off, d.version))
        return std::nullopt;
    if (!read_u8(buf, off, d.flags))
        return std::nullopt;
    if (!read_u32(buf, off, payloadSize))
        return std::nullopt;
    if (off + payloadSize > buf.size())
        return std::nullopt;
...
std::optional<ChunkDelta> ChunkDelta::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::ChunkDelta))
        return std::nullopt;
    ...
    if (!read_i32(buf, off, d.chunkZ))
        return std::nullopt;
    if (!read_u64(buf, off, d.resultingVersion))
        return std::nullopt;
    if (!read_u32(buf, off, count))
        return std::nullopt;
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ChunkDeltaOp op;
        if (!read_u8(buf, off, op.x))
            return std::nullopt;
        if (!read_u8(buf, off, op.y))
            return std::nullopt;
        if (!read_u8(buf, off, op.z))
            return std::nullopt;
        if (!read_u8(buf, off, op.blockId))
            return std::nullopt;
...
std::optional<ChunkUnload> ChunkUnload::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::ChunkUnload))
        return std::nullopt;

    ChunkUnload p;
    if (!read_i32(buf, off, p.chunkX))
        return std::nullopt;
    if (!read_i32(buf, off, p.chunkY))
        return std::nullopt;
    if (!read_i32(buf, off, p.chunkZ))
        return std::nullopt;
    return p;
}
...
std::optional<BlockPlaceRequest> BlockPlaceRequest::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::BlockPlaceRequest))
        return std::nullopt;

    BlockPlaceRequest request{};
    if (!read_u32(buf, off, request.requestId))
        return std::nullopt;
    if (!read_u16(buf, off, editCount))
        return std::nullopt;
    if (editCount > kMaxBlockPlaceEditsPerRequest)
        return std::nullopt;
    if (editCount > ((buf.size() - off) / 13))
        return std::nullopt;
    ...
    for (uint16_t i = 0; i < editCount; ++i) {
        BlockPlaceEdit edit{};
        if (!read_i32(buf, off, edit.worldX))
            return std::nullopt;
        if (!read_i32(buf, off, edit.worldY))
            return std::nullopt;
        if (!read_i32(buf, off, edit.worldZ))
            return std::nullopt;
        if (!read_u8(buf, off, edit.blockId))
            return std::nullopt;
...
std::vector<uint8_t> BlockPlaceResult::serialize() const {
    const size_t chunkCount =
        std::min<size_t>(correctiveChunks.size(), std::numeric_limits<uint16_t>::max());
    std::vector<uint8_t> out;
...
std::optional<BlockPlaceResult> BlockPlaceResult::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::BlockPlaceResult))
        return std::nullopt;

    BlockPlaceResult result{};
    if (!read_u32(buf, off, result.requestId))
        return std::nullopt;
    if (!read_u8(buf, off, accepted))
        return std::nullopt;
    if (!read_u8(buf, off, rejectRaw))
        return std::nullopt;
    if (!read_u16(buf, off, chunkCount))
        return std::nullopt;
    if (rejectRaw > static_cast<uint8_t>(BlockPlaceRejectReason::ServerError))
        return std::nullopt;
    if (chunkCount > ((buf.size() - off) / 12))
        return std::nullopt;
    ...
    for (uint16_t i = 0; i < chunkCount; ++i) {
        ...
        if (!read_i32(buf, off, chunk.chunkX))
            return std::nullopt;
        if (!read_i32(buf, off, chunk.chunkY))
            return std::nullopt;
        if (!read_i32(buf, off, chunk.chunkZ))
            return std::nullopt;
...
std::optional<BlockBreakRequest> BlockBreakRequest::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::BlockBreakRequest))
        return std::nullopt;
    ...
    if (!read_u16(buf, off, editCount))
        return std::nullopt;
    if (editCount > kMaxBlockBreakEditsPerRequest)
        return std::nullopt;
    if (editCount > ((buf.size() - off) / 12))
        return std::nullopt;
...
std::optional<BlockBreakResult> BlockBreakResult::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::BlockBreakResult))
        return std::nullopt;
    ...
    if (!read_u16(buf, off, chunkCount))
        return std::nullopt;
    if (rejectRaw > static_cast<uint8_t>(BlockBreakRejectReason::ServerError))
        return std::nullopt;
    if (chunkCount > ((buf.size() - off) / 12))
        return std::nullopt;
...
std::optional<InventoryActionRequest>
InventoryActionRequest::deserialize(const std::vector<uint8_t> &buf) {
    size_t off = 0;
    uint8_t type = 0;
    uint8_t actionTypeRaw = 0;
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::InventoryActionRequest))
        return std::nullopt;

    InventoryActionRequest req{};
    if (!read_u32(buf, off, req.requestId))
        return std::nullopt;
    if (!read_u32(buf, off, req.expectedRevision))
        return std::nullopt;
    if (!read_u8(buf, off, actionTypeRaw))
        return std::nullopt;
    if (!read_u16(buf, off, req.action.sourceSlot))
        return std::nullopt;
    if (!read_u16(buf, off, req.action.destinationSlot))
        return std::nullopt;
    if (!read_u16(buf, off, req.action.amount))
        return std::nullopt;
    if (off != buf.size())
        return std::nullopt;
    if (actionTypeRaw > static_cast<uint8_t>(InventoryActionType::Use))
        return std::nullopt;
...
std::optional<InventoryActionResult>
InventoryActionResult::deserialize(const std::vector<uint8_t> &buf) {
    size_t off = 0;
    uint8_t type = 0;
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::InventoryActionResult))
        return std::nullopt;

    InventoryActionResult result{};
    if (!read_u32(buf, off, result.requestId))
        return std::nullopt;
    if (!read_u8(buf, off, acceptedRaw))
        return std::nullopt;
    if (!read_u8(buf, off, rejectRaw))
        return std::nullopt;
    if (!read_u32(buf, off, result.newRevision))
        return std::nullopt;
    if (!read_u16(buf, off, changedCount))
        return std::nullopt;
    if (changedCount > ((buf.size() - off) / 2))
        return std::nullopt;

    if (rejectRaw > static_cast<uint8_t>(InventoryRejectReason::NotUsable))
        return std::nullopt;
...
std::optional<InventorySnapshot> InventorySnapshot::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::InventorySnapshot))
        return std::nullopt;

    InventorySnapshot snapshot{};
    if (!read_u32(buf, off, snapshot.revision))
        return std::nullopt;
    if (!read_u16(buf, off, slotCount))
        return std::nullopt;
    if (slotCount > ((buf.size() - off) / 4))
        return std::nullopt;
    if (slotCount != kInventorySlotCount)
        return std::nullopt;
    ...
    for (uint16_t i = 0; i < slotCount; ++i) {
        Slot slot{};
        if (!read_u16(buf, off, slot.itemId))
            return std::nullopt;
        if (!read_u16(buf, off, slot.quantity))
            return std::nullopt;
...
std::optional<WorldItemSnapshot> WorldItemSnapshot::deserialize(const std::vector<uint8_t> &buf) {
    ...
    if (!read_u8(buf, off, type))
        return std::nullopt;
    if (type != static_cast<uint8_t>(PacketType::WorldItemSnapshot))
        return std::nullopt;

    WorldItemSnapshot snapshot{};
    if (!read_u32(buf, off, snapshot.serverTick))
        return std::nullopt;
    if (!read_u16(buf, off, itemCount))
        return std::nullopt;
    if (itemCount > ((buf.size() - off) / kEntrySize))
        return std::nullopt;
    ...
    for (uint16_t i = 0; i < itemCount; ++i) {
        WorldItemState item{};
        if (!read_u64(buf, off, item.id))
            return std::nullopt;
        if (!read_u16(buf, off, item.itemId))
            return std::nullopt;
        if (!read_u16(buf, off, item.quantity))
            return std::nullopt;
        if (!read_f32(buf, off, item.px))
            return std::nullopt;
        if (!read_f32(buf, off, item.py))
            return std::nullopt;
        if (!read_f32(buf, off, item.pz))
            return std::nullopt;
        if (!read_f32(buf, off, item.vx))
            return std::nullopt;
        if (!read_f32(buf, off, item.vy))
            return std::nullopt;
        if (!read_f32(buf, off, item.vz))
            return std::nullopt;
