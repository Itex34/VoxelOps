...
namespace Shared::HitboxCache {
    ...
    bool Load(
        ...
        if (version != kVersion)
            return false;
        if (!ReadBinary(in, refHeight))
            return false;
        if (!ReadBinary(in, refRadius))
            return false;
        ...
        for (uint32_t i = 0; i < count; ++i) {
            Record rec;
            if (!ReadBinary(in, rec.minX))
                return false;
            if (!ReadBinary(in, rec.minY))
                return false;
            if (!ReadBinary(in, rec.minZ))
                return false;
            if (!ReadBinary(in, rec.maxX))
                return false;
            if (!ReadBinary(in, rec.maxY))
                return false;
            if (!ReadBinary(in, rec.maxZ))
                return false;
            if (!ReadBinary(in, rec.region))
                return false;
