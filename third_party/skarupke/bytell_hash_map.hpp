...
namespace ska {
    ...
    namespace detailv8 {
        ...
        public:
            ...
            sherwood_v8_table(
                size_type bucket_count, const ArgumentHash &hash, const ArgumentAlloc &alloc
            )
            ...
            sherwood_v8_table(
                std::initializer_list<T> il, size_type bucket_count, const ArgumentAlloc &alloc
            )
                    ...
                    if (first) {
                        if ((metadata & Constants::bits_for_direct_hit) !=
                            ...
                            return emplace_direct_hit(
                                {index, block}, std::forward<Key>(key), std::forward<Args>(args)...
                            );
                        first = false;
                    }
                    if (compares_equal(key, block->data[index_in_block]))
            ...
            void rehash(size_t num_items) {
                ...
                if (num_items % BlockSize)
                    ++old_num_blocks;
                for (BlockPointer it = new_buckets, end = new_buckets + old_num_blocks; it != end;
                     ++it) {
                    for (int i = 0; i < BlockSize; ++i) {
            ...
            iterator erase(const_iterator begin_it, const_iterator end_it) {
                ...
                std::sort(
                    depth_in_chain.begin(), depth_in_chain.end(), [](const auto &a, const auto &b) {
                        return a.first < b.first;
                    }
                );
                for (auto it = depth_in_chain.rbegin(), end = depth_in_chain.rend(); it != end;
                     ++it) {
                    erase(it->second.it());
                }
    ...
    emplace_new_key(LinkedListIt parent, Args &&...args) {
                ...
                AllocatorTraits::construct(
                    *this, std::addressof(*free_block.second), std::forward<Args>(args)...
                );
                free_block.second.set_metadata(Constants::magic_for_list_entry);
