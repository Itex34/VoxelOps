...
namespace ska {
    ...
    emplace_new_key(int8_t distance_from_desired, EntryPointer current_entry, Key &&key,
                ...
                if (num_slots_minus_one == 0 || distance_from_desired == max_lookups ||
                    ...
                    current_entry->emplace(
                        distance_from_desired, std::forward<Key>(key), std::forward<Args>(args)...
                    );
                    ++num_elements;
    ...
    struct prime_number_hash_policy {
        ...
        mod_function next_size_over(size_t &size) const {
            ...
            // you now have large gaps which you would hit if somebody called reserve() with an
            // unlucky number.
    ...
    class flat_hash_map : public detailv3::sherwood_v3_table<
                              std::pair<K, V>,
                              K,
                              H,
