...
std::unique_ptr<ISkyBackend> createSkyBackendForTier(GraphicsBackend backendTier,
...
void App::configureBackendPolicy(Runtime &runtime) {
    ...
    runtime.supportsGL43Shaders = (runtime.renderer->getOpenGLVersionMajor() > 4) ||
                                  (runtime.renderer->getOpenGLVersionMajor() == 4 &&
                                   runtime.renderer->getOpenGLVersionMinor() >= 3);
    ...
    std::cout << "[App] Render API: " << runtime.renderer->getApiName()
...
void App::initRenderResources(Runtime &runtime) {
    ...
    runtime.gunShader = std::make_unique<Shader>(playerVertPath.c_str(), playerFragPath.c_str());
    runtime.sky =
        createSkyBackendForTier(runtime.renderer->getActiveBackend(), skyVertPath, skyFragPath);
...
void App::initNetworking(Runtime &runtime) {
    ...
    if (!m_RequestedUsername.empty()) {
        const size_t copyLen =
            std::min(m_RequestedUsername.size(), runtime.pendingUsernameInput.size() - 1);
