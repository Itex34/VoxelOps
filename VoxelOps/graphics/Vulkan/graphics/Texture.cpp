...
TextureData loadTextureFromFile(const std::string& texturePath) {
    ...
    stbi_uc* loadedPixels = stbi_load(texturePath.c_str(), &width, &height, &channels, STBI_rgb_alpha);
    if (!loadedPixels || width <= 0 || height <= 0) {
        ...
        fallback.pixels.resize(static_cast<size_t>(fallback.width) * fallback.height * kBytesPerPixelRgba8);
...
TextureArrayData buildFallbackAtlasArray(uint32_t tilesPerAxis) {
    ...
    fallback.pixels.resize(
        static_cast<size_t>(fallback.width) *
        fallback.height *
        kBytesPerPixelRgba8 *
        ...
        for (uint32_t y = 0; y < fallback.height; ++y) {
            for (uint32_t x = 0; x < fallback.width; ++x) {
                ...
                const size_t pixelIndex =
                    ...
                    kBytesPerPixelRgba8;
                fallback.pixels[pixelIndex + 0] = checker ? tint : static_cast<uint8_t>(255u - tint);
...
TextureArrayData loadAtlasArrayFromFile(const std::string& texturePath, uint32_t tilesPerAxis) {
    ...
    if (width != height || (width % static_cast<int>(tilesPerAxis)) != 0) {
        std::cerr
            << "Warning: atlas texture dimensions " << width << "x" << height
    ...
    const uint32_t tileResolution = static_cast<uint32_t>(width / static_cast<int>(tilesPerAxis));
    ...
    for (uint32_t tileY = 0; tileY < tilesPerAxis; ++tileY) {
        for (uint32_t tileX = 0; tileX < tilesPerAxis; ++tileX) {
            ...
            for (uint32_t row = 0; row < tileResolution; ++row) {
                ...
                const size_t srcOffset = srcY * sourceRowBytes + static_cast<size_t>(tileX) * tileRowBytes;
                const size_t dstOffset = static_cast<size_t>(layer) * tilePixelBytes + row * tileRowBytes;
                std::memcpy(out.pixels.data() + dstOffset, loadedPixels + srcOffset, tileRowBytes);
            }
        }
    }
...
void VkTexture::initFromFile(
    ...
    UploadContext::StagingBuffer stagingBuffer = uploadContext.createStagingBuffer(
        physicalDevice,
    ...
    uploadContext.submitImageUpload(std::move(stagingBuffer), *m_image, textureData.width, textureData.height);
    ...
    samplerInfo.maxAnisotropy = samplerAnisotropyEnabled ? std::min(16.0f, maxSamplerAnisotropy) : 1.0f;
