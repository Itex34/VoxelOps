...
public:
    ...
    const std::vector<VkMesh>& getMeshes() const { return m_meshes; }
    std::vector<VkMesh>& getMeshes() { return m_meshes; }
    const std::vector<std::string>& getMeshTexturePaths() const { return m_meshTexturePaths; }

    bool hasLocalBounds() const { return m_hasLocalBounds; }
    const glm::vec3& getLocalMinBounds() const { return m_localMinBounds; }
    const glm::vec3& getLocalMaxBounds() const { return m_localMaxBounds; }
    const std::vector<VkModelLocalTriangle>& getLocalTriangles() const { return m_localTriangles; }
