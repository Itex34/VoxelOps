...
GiLightingSample sampleGiLighting(vec3 worldPos, vec3 normal) {
    ...
    float occlusion = clamp(depthMean * 1.3, 0.2, 1.0);
    outSample.irradiance = gi * occlusion;
...
void main() {
    ...
    vec4 texel = texture(texSampler, vec3(uvLayer, float(inTileIndex)));
    ...
    if (pathTracingEnabled) {
        ...
        vec4 directReservoir = applyRestirDiTemporal(
            vec3(computePathTracedDirectSun(inWorldPos, normalWs, sunDir)),
            inWorldPos,
            normalWs,
            validationDepth,
            currentAlbedoLuma,
        ...
        imageStore(
            ...
            vec4(octEncode(normalWs), validationDepth, currentAlbedoLuma)
        );
