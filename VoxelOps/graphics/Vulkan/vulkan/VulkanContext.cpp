...
VKAPI_ATTR vk::Bool32 VKAPI_PTR debugCallback(
	...
	const char* message = (pCallbackData && pCallbackData->pMessage) ? pCallbackData->pMessage : "No validation message.";
...
vk::PresentModeKHR choosePresentMode(const std::vector<vk::PresentModeKHR>& presentModes) {
    ...
    vk::PresentModeKHR preferredMode = benchmarkMode
...
void VulkanContext::createInstance() {
	...
	if (!m_validationEnabled) {
		layers.clear();
		if (wantValidation) {
			std::cerr << "Validation layer VK_LAYER_KHRONOS_validation not found. Continuing without validation.\n";
		}
	}
	else {
...
void VulkanContext::pickPhysicalDevice() {
	...
	if (bestDevice == nullptr) {
		if (!missingSwapchainExtensionDevices.empty()) {
			...
			oss << "No suitable GPU found. Required device extension " << VK_KHR_SWAPCHAIN_EXTENSION_NAME << " is missing on: ";
...
bool VulkanContext::hasAdequateSwapchainSupport(const vk::raii::PhysicalDevice& device, const vk::raii::SurfaceKHR& surface) {
	auto formats = device.getSurfaceFormatsKHR(surface);
	auto presentModes = device.getSurfacePresentModesKHR(surface);
	return !formats.empty() && !presentModes.empty();
}

QueueFamilyIndices VulkanContext::findQueueFamilies(const vk::raii::PhysicalDevice& device, const vk::raii::SurfaceKHR& surface) {
...
void VulkanContext::createDevice() {
    ...
    if (supportedFeatures.fragmentStoresAndAtomics == VK_TRUE) {
        ...
        std::cerr
            << "[Vulkan] fragmentStoresAndAtomics not supported; fragment storage image writes are unavailable.\n";
    }
    ...
    const bool rtExtensionsAvailable =
    ...
    supportedComputeDerivatives.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_COMPUTE_SHADER_DERIVATIVES_FEATURES_KHR;
#endif
...
#endif
    vkGetPhysicalDeviceFeatures2(static_cast<VkPhysicalDevice>(*physicalDevice), &supportedFeatures2);
    ...
    std::cout
        << "[Vulkan] hardware ray tracing support="
    ...
    std::cout
        << "[Vulkan] compute shader derivatives support="
    ...
    enabledComputeDerivatives.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_COMPUTE_SHADER_DERIVATIVES_FEATURES_KHR;
...
void VulkanContext::createSwapchain(uint32_t windowWidth, uint32_t windowHeight) {
    ...
    std::cout
        << "[Vulkan] Swapchain present mode: " << presentModeToString(presentMode)
        << " | images: " << imageCount
...
vk::raii::ImageView VulkanContext::createImageView(vk::Image image, vk::Format format, vk::ImageAspectFlags aspectFlags) {
