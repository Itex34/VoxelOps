...
bool Renderer::ensureChunkAndSkyResources() {
    if (m_ChunkShader && m_SkyBackend && m_textureAtlas.getArrayTextureId() != 0) {
        return true;
    }
...
bool Renderer::renderSunShadowMaps(RenderFrameParams &params, const glm::mat4 &projection,
    ...
    const glm::vec3 sunDir = params.sunDirection;
    ...
    for (int cascadeIndex = 0; cascadeIndex < kSunShadowCascadeCount; ++cascadeIndex) {
        ...
        m_chunkScene.renderChunksDepthPass(m_SunShadowProgram, lightViewProjVoxel, shadowCenter,
...
void Renderer::renderFrame(RenderFrameParams &params) {
    if (params.debugShader == nullptr) {
        static bool loggedMissingFrameResources = false;
        if (!loggedMissingFrameResources) {
            std::cerr << "[Renderer] Skipping frame: missing OpenGL frame resources.\n";
            loggedMissingFrameResources = true;
        }
        return;
    }
    if (!ensureChunkAndSkyResources() || m_ChunkShader == nullptr || m_SkyBackend == nullptr) {
    ...
    Shader &debugShader = *params.debugShader;
    ISkyBackend &sky = *m_SkyBackend;
    m_chunkScene.syncFromChunkManager(params.chunkManager);
    ...
    bool useExternalSceneTextures = sky.requiresExternalSceneTextures();
    ...
    if (useExternalSceneTextures && sky.supportsExternalShadowMap()) {
    ...
    chunkShader.setInt("uOutputHdrLinear", useExternalSceneTextures ? 1 : 0);
    ...
    chunkShader.setInt("uUseSunShadowMap", useChunkSunShadow ? 1 : 0);
    ...
    chunkShader.setFloat("uSunShadowLowSunBiasBoost",
    ...
    const bool useBakedSunChannel =
        !sky.requiresExternalSceneTextures() || !useChunkSunShadow;
    chunkShader.setInt("uUseBakedSunChannel", useBakedSunChannel ? 1 : 0);
    if (useChunkSunShadow) {
        ...
        chunkShader.setMat4("uSunShadowViewProjNear", m_LastSunShadowViewProjVoxel[0]);
        chunkShader.setMat4("uSunShadowViewProjFar", m_LastSunShadowViewProjVoxel[1]);
        const float texelSize = 1.0f / static_cast<float>(std::max(1, m_SunShadowMapSize));
        chunkShader.setVec2("uSunShadowTexelSizeNear", glm::vec2(texelSize));
        chunkShader.setVec2("uSunShadowTexelSizeFar", glm::vec2(texelSize));
        chunkShader.setFloat("uSunShadowSplitDepthKm", m_SunShadowCascadeFarMeters[0] * 0.001f);
        chunkShader.setFloat("uSunShadowBlendKm", 0.012f);
    } else {
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, 0);
        glActiveTexture(GL_TEXTURE2);
    ...
    if (useExternalSceneTextures) {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        glViewport(0, 0, frameWidth, frameHeight);
        sky.setExternalSceneTextures(m_SceneColorTex, m_SceneLinearDepthTex);
        if (useExternalShadowMap) {
            sky.setExternalShadowMap(m_SunShadowDepthTex[0], externalShadowViewProj);
        } else {
            sky.clearExternalShadowMap();
        }
        drainPendingRendererGlErrors("external sky input setup");
        m_skyPass.compositeExternal(sky, params.activeCamera, projection, view);
        sky.clearExternalSceneTextures();
        sky.clearExternalShadowMap();
    }
