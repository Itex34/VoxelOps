...
namespace {
    void drainStageErrors(const char *stageTag) {
        ...
        if (count > 0) {
            ...
            if (loggedCount < 16) {
                std::cerr << "[OpenGLWorldPass] Cleared " << count << " GL error(s) after "
...
void OpenGLWorldPass::execute(const OpenGLWorldPassInput &in) const {
    Frustum frustum;
    frustum.extractPlanes(in.cullingViewProjection);

    glPolygonMode(GL_FRONT_AND_BACK, in.toggleWireframe ? GL_LINE : GL_FILL);
    drainStageErrors("pre world draw");

    in.chunkScene.renderChunks(
        in.chunkShader,
        frustum,
        in.frameParams.cullingCamera ? in.frameParams.cullingCamera->position
                                     : in.frameParams.activeCamera.position,
        in.frameParams.chunkRenderDistance
    );
    drainStageErrors("chunk world pass");

    const glm::vec3 ambientColor = glm::vec3(0.36f, 0.40f, 0.46f);
    in.remotePlayerRenderer.render(
        in.frameParams.localPlayerPosition,
        in.frameParams.remotePlayers,
        in.view,
        in.projection,
        in.lightDir,
        in.lightColor,
