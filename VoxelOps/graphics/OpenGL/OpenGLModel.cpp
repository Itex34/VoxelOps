...
float OpenGLModel::getLocalMinY() const noexcept {
    return m_hasLocalBounds ? m_localMinBounds.y : 0.0f;
}

float OpenGLModel::getLocalHeight() const noexcept {
    if (!m_hasLocalBounds) {
        return 1.0f;
    }
...
glm::vec3 OpenGLModel::getLocalSize() const noexcept {
    if (!m_hasLocalBounds) {
...
glm::vec3 OpenGLModel::getLocalMinBounds() const noexcept {
    return m_hasLocalBounds ? m_localMinBounds : glm::vec3(0.0f);
}

glm::vec3 OpenGLModel::getLocalMaxBounds() const noexcept {
    return m_hasLocalBounds ? m_localMaxBounds : glm::vec3(0.0f);
}

const ModelRegionAabb &OpenGLModel::getLocalRegionAabb(ModelRegion region) const noexcept {
    return m_localRegionAabbs[static_cast<size_t>(region)];
}

const std::vector<ModelLocalTriangle> &OpenGLModel::getLocalTriangles() const noexcept {
...
OpenGLModel::processMesh(aiMesh *mesh, const aiScene *scene, const aiMatrix4x4 &nodeTransform) {
    ...
    if (mesh->mMaterialIndex >= 0) {
        ...
        std::vector<OpenGLModelTexture> diffuseMaps =
...
unsigned int OpenGLModel::TextureFromFile(const char *path, const std::string &directory) {
...
unsigned int OpenGLModel::TextureFromAssimp(const aiTexture *aiTex) {
    unsigned int textureID;
    glGenTextures(1, &textureID);
    glBindTexture(GL_TEXTURE_2D, textureID);
...
void OpenGLModel::finalizeRegionAabbsFromVertices() {
