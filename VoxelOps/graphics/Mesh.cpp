...
void Mesh::draw() const {
    ...
    if (glIsVertexArray(VAO) != GL_TRUE) {
        ...
        if (s_invalidVaoLogCount < 8) {
            std::cerr
                << "[mesh] skipped draw: invalid VAO=" << VAO
                << " EBO=" << EBO
                << " indices=" << indexCount_
                << "\n";
    ...
    const size_t safeMaxTextureUnits = (maxTextureUnits > 0) ? static_cast<size_t>(maxTextureUnits) : 16u;
    ...
    if (textures.size() > bindCount) {
        ...
        if (s_textureClampLogCount < 8) {
            std::cerr
                << "[mesh] clamped bound textures from " << textures.size()
                << " to " << bindCount
                << " (max units=" << safeMaxTextureUnits << ")\n";
    ...
    if (elementBufferBinding == 0) {
        ...
        if (s_missingEboBindingLogCount < 8) {
            std::cerr
                << "[mesh] skipped draw: VAO has no element buffer binding"
