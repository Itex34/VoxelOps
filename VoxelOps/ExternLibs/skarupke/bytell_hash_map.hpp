...
template <typename T, typename FindKey, typename ArgumentHash, typename Hasher,
          typename ArgumentEqual, typename Equal, typename ArgumentAlloc, typename ByteAlloc,
          uint8_t BlockSize>
class sherwood_v8_table : private ByteAlloc, private Hasher, private Equal {
  ...
  public:
    ...
    sherwood_v8_table(const sherwood_v8_table &other)
        : sherwood_v8_table(other, AllocatorTraits::select_on_container_copy_construction(
                                       other.get_allocator())) {}
    ...
    void rehash(size_t num_items) {
        num_items = std::max(num_items, static_cast<size_t>(std::ceil(
  ...
  private:
    ...
    struct LinkedListIt {
        ...
        LinkedListIt next(sherwood_v8_table &table) const {
            ...
            size_t next_index = table.hash_policy.keep_in_range(
                index + Constants::jump_distances[distance], table.num_slots_minus_one);
    ...
    std::pair<int8_t, LinkedListIt> find_free_index(LinkedListIt parent) const {
        for (int8_t jump_index = 1; jump_index < Constants::num_jump_distances; ++jump_index) {
            size_t index = hash_policy.keep_in_range(
                parent.index + Constants::jump_distances[jump_index], num_slots_minus_one);
    ...
    void deallocate_data(BlockPointer begin, size_t num_slots_minus_one) {
        ...
        AllocatorTraits::deallocate(*this, typename AllocatorTraits::pointer(as_byte_pointer),
                                    memory);
    }
...
class bytell_hash_map
    : public detailv8::sherwood_v8_table<
          std::pair<K, V>, K, H, detailv8::KeyOrValueHasher<K, std::pair<K, V>, H>, E,
  ...
  public:
    ...
    template <typename M>
    typename Table::iterator insert_or_assign(typename Table::const_iterator, const key_type &key,
                                              M &&m) {
        return insert_or_assign(key, std::forward<M>(m)).first;
    }
    template <typename M>
    typename Table::iterator insert_or_assign(typename Table::const_iterator, key_type &&key,
                                              M &&m) {
...
class bytell_hash_set
    : public detailv8::sherwood_v8_table<
          T, T, H, detailv8::functor_storage<size_t, H>, E, detailv8::functor_storage<bool, E>, A,
          typename std::allocator_traits<A>::template rebind_alloc<unsigned char>,
