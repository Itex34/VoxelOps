...
namespace Respawning {
    namespace {
        ...
        glm::vec3 ChooseRespawnPosition(
            const std::unordered_map<PlayerID, ServerPlayer> &playersById, PlayerID respawningId
        ) {
        ...
        void
        PrewarmRespawnCollisionChunks(ChunkManager &chunkManager, const glm::vec3 &respawnPos) {
            ...
            for (int dx = -kRespawnCollisionPrewarmRadiusXZ; dx <= kRespawnCollisionPrewarmRadiusXZ;
                 ++dx) {
                for (int dz = -kRespawnCollisionPrewarmRadiusXZ;
                     dz <= kRespawnCollisionPrewarmRadiusXZ;
                     ++dz) {
                    for (int dy = -kRespawnCollisionPrewarmRadiusY;
                         dy <= kRespawnCollisionPrewarmRadiusY;
                         ++dy) {
                        const glm::ivec3 chunkPos(
                            centerChunk.x + dx, centerChunk.y + dy, centerChunk.z + dz
                        );
                        if (!chunkManager.inBounds(chunkPos) ||
                            chunkManager.hasChunkLoaded(chunkPos)) {
                            continue;
                        }
    ...
    void MarkPlayerDead(
        ServerPlayer &player, Clock::time_point now, std::chrono::milliseconds respawnDelay
    ) {
        player.health = 0.0f;
    ...
    bool TryRespawnPlayer(
        ...
        if (!player.pendingRespawnRequest || player.respawnAt == Clock::time_point{} ||
