...
namespace Shoot {
    ...
    ShootContext BuildContext(
        HSteamNetConnection connection, const ShootRequest &request, uint32_t currentServerTick
    ) {
        ShootContext ctx{};
    ...
    bool FinalizeContext(
        ...
        const ShootValidation::ValidatedOrigin validatedOrigin =
            ShootValidation::ComputeValidatedOrigin(
                chunkManager, shooterEyePos, requestPos, originTolerance, originOcclusionEpsilon
            );
