...
template <typename Result, typename... Args> struct functor_storage<Result, Result (*)(Args...)> {
...
struct KeyOrValueHasher : functor_storage<size_t, hasher> {
    ...
    template <typename F, typename S> size_t operator()(const std::pair<F, S> &value) {
        return static_cast<hasher_storage &>(*this)(value.first);
    }
    template <typename F, typename S> size_t operator()(const std::pair<F, S> &value) const {
        return static_cast<const hasher_storage &>(*this)(value.first);
    }
};
...
template <typename T> struct HashPolicySelector<T, void_t<typename T::hash_policy>> {
...
class flat_hash_map
    : public detailv3::sherwood_v3_table<std::pair<K, V>, K, H,
...
class flat_hash_set
    : public detailv3::sherwood_v3_table<T, T, H, detailv3::functor_storage<size_t, H>, E,
