...
void handleRespawn(ServerPlayer &player,
    ...
    if (player.inputBuffer.consumeNext(cmd)) {
        player.yaw = Shared::Utils::NormalizeYawDegrees(std::isfinite(cmd.yaw) ? cmd.yaw : 0.0f);
...
void handleTimeouts(std::unordered_map<PlayerID, ServerPlayer> &playersById,
...
void updatePlayers(std::unordered_map<PlayerID, ServerPlayer> &playersById,
                   std::list<PlayerID> &playersOrder,
                   double deltaSeconds,
                   ChunkManager &chunkManager,
    ...
    for (auto &entry : playersById) {
        ...
        if (player.isAlive) {
            ServerMovementSimulation::simulatePhysicsForPlayer(player, deltaSeconds, chunkManager);
        } else {
