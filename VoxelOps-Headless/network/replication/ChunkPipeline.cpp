...
uint16_t Runtime::ClampViewDistance(uint16_t requested) {
...
std::string Runtime::AllocateAutoUsernameLocked(HSteamNetConnection incomingConn) {
...
void Runtime::StartChunkPipeline() {
    {
...
void Runtime::StopChunkPipeline() {
...
bool Runtime::PrepareChunkForStreaming(const ChunkCoord &coord) {
...
void Runtime::ChunkPrepWorkerLoop() {
    while (true) {
        ChunkPrepTask task;
        {
        ...
        const ChunkPipelineKey key{task.conn, task.coord};
        {
            ...
            if (prepared && !m_chunkPrepQuit.load(std::memory_order_acquire) &&
...
bool Runtime::QueueChunkPreparation(HSteamNetConnection conn, const ChunkCoord &coord) {
...
size_t Runtime::FlushChunkSendQueueForClient(HSteamNetConnection conn, size_t maxSends) {
    size_t sent = 0;
    while (sent < maxSends) {
        ChunkCoord coord{};
        ...
        bool stillPending = false;
        {
            ...
            if (it != m_clients.end()) {
                stillPending =
                    it->second.pendingChunkData.find(coord) != it->second.pendingChunkData.end();
            }
        }
        if (!stillPending) {
            continue;
...
size_t Runtime::FlushChunkSendQueues(size_t globalBudget, size_t perClientBudget) {
...
size_t Runtime::GetChunkSendQueueDepthForClient(HSteamNetConnection conn) {
    std::lock_guard<std::mutex> lk(m_chunkPipelineMutex);
...
void Runtime::ClearChunkPipelineForConnection(HSteamNetConnection conn) {
    std::lock_guard<std::mutex> lk(m_chunkPipelineMutex);
...
bool Runtime::SendChunkData(HSteamNetConnection conn, const ChunkCoord &coord) {
...
bool Runtime::SendChunkUnload(HSteamNetConnection conn, const ChunkCoord &coord) {
    ChunkUnload packet;
    packet.chunkX = coord.x;
    packet.chunkY = coord.y;
    packet.chunkZ = coord.z;
