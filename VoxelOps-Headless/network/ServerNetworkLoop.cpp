...
void ServerNetwork::RunInboundMessagePhase(
...
)
{
    ...
    messageDrainUs = static_cast<double>(
...
uint64_t ServerNetwork::RunSimulationPhase(
...
double ServerNetwork::RunSnapshotPhase(
    uint32_t serverTick,
...
bool ServerNetwork::RunScoreboardPhase(std::chrono::steady_clock::time_point& nextScoreboardBroadcastAt)
{
...
size_t ServerNetwork::RunChunkSendPhase(
    bool simBacklog,
...
)
{
    ...
    const size_t chunksSentThisLoop = FlushChunkSendQueues(
...
void ServerNetwork::MainLoop()
{
    ...
    while (!m_quit) {
        ...
        const uint64_t simTicksThisLoop = RunSimulationPhase(simAccumulator, serverTick, simUs, simBacklog);
        ...
        const size_t chunkInterestTasksThisLoop = RunChunkInterestPhase(simBacklog, chunkInterestUs);
        const size_t chunksSentThisLoop = RunChunkSendPhase(simBacklog, nextChunkSendFlushAt, chunkSendUs);
        const size_t collisionPrewarmGeneratedThisLoop = RunCollisionPrewarmPhase(
            simBacklog,
            nextCollisionPrewarmAt,
            collisionPrewarmUs
        );
