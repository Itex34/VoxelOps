...
bool ServerNetwork::IsInboundRateLimitExceeded(HSteamNetConnection incoming, PacketType packetType,
                                               uint32_t bytes) {
    bool exceededRateLimit = false;
    {
        ...
        if (it != m_clients.end()) {
            ...
            exceededRateLimit = (session.inboundPacketsInWindow > kMaxInboundPacketsPerWindow) ||
...
void ServerNetwork::HandleConnectRequest(HSteamNetConnection incoming, const void *data,
                                         uint32_t size) {
...

    {
        ...
        if (it != m_clients.end() && it->second.playerId != 0 && !it->second.username.empty()) {
            ...
            sendResponse(response);
            std::cout << "[register] duplicate ConnectRequest ignored conn=" << incoming
        ...
        for (const auto &[conn, session] : m_clients) {
            ...
            if (session.identity == identity) {
                ...
                sendResponse(response);
                std::cout << "[register rejected] conn=" << incoming << " identity=" << identity
    ...
    const PlayerID playerId =
        m_playerManager.onPlayerConnect(connHandle, glm::vec3(0.0f, 60.0f, 0.0f));
...
void ServerNetwork::HandleMessagePacket(HSteamNetConnection incoming, const void *data,
                                        uint32_t size) {
