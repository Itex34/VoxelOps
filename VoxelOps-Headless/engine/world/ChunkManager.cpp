...
ChunkManager::ChunkManager(GameMode gameMode, uint64_t seed)
    ...
    decoratedChunks.reserve(m_chunks.slotCount());
}
...
void ChunkManager::updateDirtyChunks() {
    std::vector<ServerChunk *> toUpdate;
    {
        ...
        m_chunks.forEachLoaded([&](const glm::ivec3 &, ServerChunk &chunk) {
            if (chunk.dirty())
...
void ChunkManager::setBlockInWorld(const glm::ivec3 &worldPos, BlockID id) {
    ...
    if (lPos.x == 0)
        markChunkDirty(cPos + glm::ivec3(-1, 0, 0));
    if (lPos.x == CHUNK_SIZE - 1)
        markChunkDirty(cPos + glm::ivec3(1, 0, 0));
    if (lPos.y == 0)
        markChunkDirty(cPos + glm::ivec3(0, -1, 0));
    if (lPos.y == CHUNK_SIZE - 1)
        markChunkDirty(cPos + glm::ivec3(0, 1, 0));
    if (lPos.z == 0)
        markChunkDirty(cPos + glm::ivec3(0, 0, -1));
    if (lPos.z == CHUNK_SIZE - 1)
        markChunkDirty(cPos + glm::ivec3(0, 0, 1));
}
...
void ChunkManager::setBlockGlobal(int worldX, int worldY, int worldZ, BlockID id) {
    ...
    {
        std::shared_lock<std::shared_mutex> lk(mapMutex);
        chunkPtr = m_chunks.get(chunkPos);
    }
...
bool ChunkManager::hasChunkLoaded(const glm::ivec3 &chunkPos) const {
    ...
    return m_chunks.containsLoaded(chunkPos);
}
...
glm::ivec3 ChunkManager::worldToChunkPos(const glm::ivec3 &wp) const {
    return glm::ivec3(
        floorDiv(wp.x, CHUNK_SIZE), floorDiv(wp.y, CHUNK_SIZE), floorDiv(wp.z, CHUNK_SIZE)
    );
}
...
void ChunkManager::markChunkDirty(const glm::ivec3 &pos) {
    ...
    {
        std::shared_lock<std::shared_mutex> lk(mapMutex);
        chunkPtr = m_chunks.get(pos);
    }
    if (chunkPtr)
        chunkPtr->markDirty();
}

std::unordered_map<glm::ivec3, ServerChunk *, IVec3Hash, IVec3Eq>
ChunkManager::snapshotChunkMap() const {
...
ServerChunk *ChunkManager::loadOrGenerateChunk(const glm::ivec3 &chunkPos) {
    ...
    // quick path: check if present
    {
        ...
        ServerChunk *chunk = m_chunks.get(chunkPos);
        if (chunk != nullptr) {
