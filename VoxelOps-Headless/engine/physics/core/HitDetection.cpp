...
namespace {
    ...
    const std::string &SharedHitboxCachePath() {
        static const std::string kPath =
            Shared::RuntimePaths::ResolveSharedPath("generated/player_hitboxes.bin")
                .generic_string();
        return kPath;
    }
    ...
    const std::string &SharedMeshHitCachePath() {
        static const std::string kPath =
            Shared::RuntimePaths::ResolveSharedPath("generated/player_mesh_hit.bin")
                .generic_string();
        return kPath;
    }
    ...
    GetSharedPlayerHitboxes(float &outReferenceHeight, float &outReferenceRadius) {
        ...
        std::call_once(initFlag, [&]() {
            ...
            if (Shared::HitboxCache::Load(
                    SharedHitboxCachePath(), loadedHeight, loadedRadius, records
                ) &&
                !records.empty()) {
            ...
            if (cachedHitboxes.empty()) {
                ...
                std::cerr
                    << "[hitbox] cache missing/unusable, using fallback procedural hitboxes.\n";
                if (kEnableHitboxDiagnostics) {
    ...
    CachedTransform BuildPlayerTransform(const glm::vec3 &position, float yaw) {
        ...
        transform.model = glm::rotate(
            transform.model,
            glm::radians(Rules::ToModelYawDegrees(yaw)),
        ...
        for (const ServerPlayerCombatSnapshot &target : *input.players) {
            ...
            if (!meshTriangles.empty() && meshBroadphase.valid) {
                ...
                broadphaseCenterWorld =
                    glm::vec3(targetTransform.model * glm::vec4(meshCenterLocal, 1.0f));
                broadphaseRadiusWorld =
                    meshBroadphase.radiusLocal * uniformScale + input.hitboxPadXZ;
            } else {
            ...
            if (!meshTriangles.empty()) {
                ...
                for (const Hitbox &base : *baseHitboxes) {
                    ...
                    scaled.max = glm::vec3(
                        base.max.x * sx + input.hitboxPadXZ,
            ...
            const HitRegion resolvedRegion =
                hit.hit ? ResolveHitRegionByPoint(
                              hit.hitPointWorld, targetTransform.invModel, targetHeight, hit.region
                          )
                        : HitRegion::Unknown;
            if (input.enableValidationLogs && hit.hit) {
                std::cout << "[shoot/validate] candidate player=" << target.id
                          << " dist=" << hit.distance
